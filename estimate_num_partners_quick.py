...
if __name__ == '__main__':
    ...
    coeffs = np.mean(results[2], axis=1).values
    overall_roc = np.mean(results[0])
    overall_precision = np.mean(results[1])
    feature_coeff_tuples = list(zip(feature_frame.columns.values, coeffs))

    # use coefficients to sort individual features
    feature_coeff_tuples.sort(key=lambda x:x[1], reverse=True)
    sorted_features = [x[0] for x in feature_coeff_tuples]
    print('overall ROC:', overall_roc, 'overall precision:', overall_precision)
    ...
    for i in range(len(sorted_features)):
        ...
        print('num features used:', num_features, 'ROC:', sub_roc, 'Precision:', sub_precision, 'training_time:', end - start)
        if sub_roc >= overall_roc - 0.05:
            break

    # write performance of classifiers
    ff_performance_file = open(output_path + '/ff_performance.tsv', 'w')
